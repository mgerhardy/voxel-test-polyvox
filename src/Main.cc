...
public:
    ...
    int bake_geom(const GeomMesher::Result& meshResult, const glm::vec3& Scale, const glm::vec3& Translate);
    void handle_input();
...
//------------------------------------------------------------------------------
int
VoxelTest::bake_geom(const GeomMesher::Result& meshResult, const glm::vec3& Scale, const glm::vec3& Translate) {
    if (meshResult.getNoOfVertices() > 0) {
        int geomIndex = this->geomPool.Alloc();
        auto& geom = this->geomPool.Geoms[geomIndex];
        Log::Info("Indices: %i, vertices: %i", meshResult.getNoOfIndices(), meshResult.getNoOfVertices());
        Gfx::UpdateIndices(geom.Mesh, meshResult.getRawIndexData(), meshResult.getNoOfIndices() * sizeof(typename GeomMesher::Result::IndexType));
        Gfx::UpdateVertices(geom.Mesh, meshResult.getRawVertexData(), meshResult.getNoOfVertices() * sizeof(typename GeomMesher::Result::VertexType));
        geom.NumVertices = meshResult.getNoOfVertices();
        geom.VSParams.Model = glm::mat4();
        geom.VSParams.LightDir = this->lightDir;
        geom.VSParams.Scale = Scale;
        geom.VSParams.Translate = Translate;
        geom.VSParams.TexTranslate = glm::vec3(0.0f, 0.0f, 0.0f);
        return geomIndex;
    }
    else {
...
VoxelTest::OnRunning() {
    ...
    if (!this->visTree.geomGenJobs.Empty()) {
        ...
        while ((numProcessedJobs < MaxChunksGeneratedPerFrame) && !this->visTree.geomGenJobs.Empty()) {
            ...
            const GeomMesher::Result& meshResult = this->geomMesher.Meshify(vol, job.Translate, job.Scale);
            int geom = this->bake_geom(meshResult, job.Scale, job.Translate);
